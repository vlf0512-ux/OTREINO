...
window.onload = () => {
  ...
  if (semanaSalva !== semanaAtual) {
    ...
    localStorage.removeItem("diasFinalizados");
    diasFinalizados = {};
  }
  ...
  document.getElementById("boxAdicionar").style.display = "none";
...
function mostrarTreino() {
  ...
  const boxAdicionar = document.getElementById("boxAdicionar");
  ...
  if (!dia) {
    ...
    boxAdicionar.style.display = "none";
    return;
  }

  inicio.style.display = "none";
  boxAdicionar.style.display = "block";
...
function atualizarContador(dia) {
  ...
  const contador = document.getElementById(`contador-${dia}`);
  if (contador) {
    contador.textContent = total ? `— ${total} exercícios` : "";
  }
}
